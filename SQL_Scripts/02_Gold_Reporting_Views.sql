...
customer_orders AS (
    SELECT
        customer_unique_id,
        COUNT(customer_id) AS lifetime_orders
...
SELECT DISTINCT
    ...
    co.lifetime_orders AS [Total Lifetime Orders],
    CASE 
        WHEN co.lifetime_orders > 1 THEN 'Returning'
        ELSE 'New'
...
SELECT
    ...
    ISNULL(REPLACE(t.product_category_name_english, '_', ' '), 'Unknown') AS [Product Category Name English],
...
-- =============================================================================
-- Create Dimension: v_DimDate (Independent Calendar Generation)
-- =============================================================================
...
WITH RECURSIVE_CALENDAR AS (
    SELECT CAST('2017-01-01' AS DATE) AS full_date
    UNION ALL
    SELECT DATEADD(DAY, 1, full_date)
    FROM RECURSIVE_CALENDAR
    WHERE full_date < '2018-08-31'
)
SELECT 
    CONVERT(INT, FORMAT(full_date, 'yyyyMMdd')) AS [Date Key],
